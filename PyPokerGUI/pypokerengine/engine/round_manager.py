...
from game_customization import GameCustomization
...
class RoundManager:
  ...
  def __deal_holecard(self, deck, players):
    for player in players:
      player.add_holecard(deck.draw_cards(GameCustomization.hole_card_count))
  ...
  def __start_street(self, state):
    ...
    if street == Const.Street.PREFLOP:
      ...
      if GameCustomization.has_turn:
        return self.__turn(state)
      else:
        return self.__showdown(state)
    elif GameCustomization.has_river and street == Const.Street.RIVER:
      if GameCustomization.has_turn:
  ...
  def __flop(self, state):
    for card in state["table"].deck.draw_cards(GameCustomization.public_card_count):
